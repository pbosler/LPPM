...
int main(){
	...
	vtkLookupTable * latScale = vtkLookupTable::New();
		...
		latScale -> SetNumberOfTableValues(numColorsInColorbars);
		range = latMax - latMin;
		for (int i = 0; i < numColorsInColorbars; i++)
		{
			val = latMin + ( (double)i/numColorsInColorbars) * range;
			//GetColorForValue_NCLDetail( val, r, g, b, latMin, latMax);
			GetColorForValue_RainbowDesaturated( val, r, g, b, latMin, latMax);
	...
	vtkPolyDataMapper * meshMapper = vtkPolyDataMapper::New();
		...
		meshMapper -> SetScalarRange(latMin, latMax);
		meshMapper -> SetLookupTable(latScale);
	...
	vtkScalarBarActor * relVortColorbar = vtkScalarBarActor::New();
		...
		relVortColorbar -> SetPosition2(0.3, 0.9);
		relVortColorbar -> SetWidth(0.25);
		// relVortColorbar -> SetAnnotationTextScaling(0); // only in VTK 6.0 and later
	...
	vtkScalarBarActor * latColorbar = vtkScalarBarActor::New();
		...
		latColorbar -> SetPosition(0.03, 0.1);
