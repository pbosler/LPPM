...
!
character(len=MAX_STRING_LENGTH) :: namelistFile = 'AdvectSlotC.namelist'
...
! define tracer
!
call InitSlottedCylindersTracer(slotC, tracerID)

!
! build initial mesh
!
call New(sphere, panelKind, initNest, AMR, nTracer, ADVECTION_SOLVER)
call SetSlottedCylindersTracerOnMesh(sphere, slotC)
...
do timeJ = 0, timesteps - 1
	if ( mod( timeJ+1, remeshInterval) == 0 ) then
		...
		if ( remeshCounter < resetAlphaInterval ) then
			...
			! remesh to t = 0
			!
			call LagrangianRemeshToInitialTime(sphere, remesh, NullVorticity, nullVort, SetSlottedCylindersTracerOnMesh, slotC)
