...
type BVEMesh
	...
	type(Field) :: velocity
	type(Field), dimension(:), pointer :: tracers => null()
...
!
interface
	real(8) function scalarFnOfSpace( x, y, z)
		real(8), intent(in) :: x, y, z
	end function
end interface

interface
	function vectorFnOfSpace( x, y, z)
		real(8), dimension(3) :: vectorFnOfSpace
		real(8), intent(in) :: x, y, z
	end function
end interface
...
subroutine newPrivate( self, meshSeed, initNest, maxNest, amrLimit, sphereRadius, rotationRate )
	...
	call New(self%absVort, 1, self%mesh%particles%N, "absVort", "1/s")
	call New(self%relVort, 1, self%mesh%particles%N, "relVort", "1/s")
	call New(self%absStream, 1, self%mesh%particles%N, "absStreamFn", "m^2/s")
	call New(self%relStream, 1, self%mesh%particles%N, "relStreamFn", "m^2/s")
	call New(self%velocity, 3, self%mesh%particles%N, "velocity", "m/s")
...
subroutine SetInitialVorticityOnMesh(self, relVortFn )
	type(BVEMesh), intent(inout) :: self
	procedure(scalarFnOfSpace) :: relVortFn
	integer(kint) :: i
	real(kreal) :: zeta
	...
	do i = 1, self%mesh%particles%N
		...
		call InsertScalarToField(self%absVort, zeta + 2.0_kreal*self%rotationRate*self%mesh%particles%z(i) / self%radius)
	enddo
end subroutine
...
subroutine SetScalarTracerOnMesh(self, tracerId, tracerFn )
	...
	procedure(scalarFnOfSpace) :: tracerFn
	integer(kint) :: i
...
subroutine SetVectorTracerOnMesh( self, tracerID, tracerFn )
	type(BVEMesh), intent(inout) :: self
	integer(kint), intent(in) :: tracerId
	procedure(vectorFnOfSpace) :: tracerFn
	integer(kint) :: i
